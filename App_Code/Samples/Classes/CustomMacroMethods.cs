...
public static class CustomMacroMethods
{
    ...
    public static void RegisterMethods()
    {
        ...
        MacroMethods.RegisterMethod("GetNombreProyecto", GetNombreProyecto, typeof(string), "Returns concatenation of two strings.", null, 1, new object[,] { { "param1", typeof(string), "First string to concatenate." }, { "param2", typeof(string), "Second string to concatenate." } }, null);
        MacroMethods.RegisterMethod("GetNombreProyectoSnippet", GetNombreProyecto, typeof(string), "Calls MyMethod on lower case current user name.", null, 0, null, null, new List<Type>() { typeof(string) }, "MyMethod(ToLower(CurrentDocument.DocumentName), |);", false);
    ...
    public static string GetNombreProyecto(string CurrentAliasPath)
    {
        try
        {
            // Tree node
            if ( CurrentAliasPath == "")
            {
                CurrentAliasPath = CMSContext.CurrentDocument.DocumentNamePath;
            }
            string parentPath = "/" + CurrentAliasPath.Split('/')[0];
            ...
            if (!String.IsNullOrEmpty(parentPath))
            {
                ...
                //node = tree.SelectSingleNode(CMSContext.CurrentSiteName, parentPath, CMS.CMSHelper.CMSContext.CurrentDocumentCulture.CultureCode, true);
                ...
                return "PROYECTO : " + CurrentAliasPath.Split('/')[3];
    ...
    public static object GetNombreProyecto(params object[] parameters)
    {
        switch (parameters.Length)
        {
            case 2:
                // Overload with one parameter
                return GetNombreProyecto(ValidationHelper.GetString(parameters[1], ""));
