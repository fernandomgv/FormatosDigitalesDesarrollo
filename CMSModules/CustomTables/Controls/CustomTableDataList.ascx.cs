...
public partial class CMSModules_CustomTables_Controls_CustomTableDataList : CMSUserControl
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        gridData.OnBeforeDataReload += new OnBeforeDataReload(gridData_OnBeforeDataReload);
        //gridData.OnAfterRetrieveData += new OnAfterRetrieveData(gridData_OnAfterRetrieveData);
    }

    void gridData_OnBeforeDataReload()
    {
        if (this.CustomTableClassInfo.ClassName == "customtable.SPATS_Cultivo")
        {
        var territorio = QueryHelper.GetInteger("territorio", 0);
        if (territorio>0)
        {
            gridData.WhereClause="territorioId="+territorio;
        }
        }
    }
