...
public partial class CMSModules_Membership_Pages_Users_User_List : CMSUsersPage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        actions[0, 2] = null;
        var territorio = QueryHelper.GetInteger("territorio", 0);
        actions[0, 3] = SiteID > 0 ? ResolveUrl("user_new.aspx?siteid=" + SiteID +"&territorio="+territorio) : ResolveUrl("user_new.aspx");
        actions[0, 4] = null;
    ...
    protected void gridElem_OnBeforeDataReload()
    {
        if (!RequestHelper.IsAsyncPostback())
        {
            ...
            gridElem.WhereCondition = userFilterElem.WhereCondition;
                ...
                if (territorio > 0)
                {
                    gridElem.WhereClause = "territorioId=" + territorio;
                }
    ...
    protected void gridElem_OnAction(string actionName, object actionArgument)
    {
        if (actionName == "edit")
        {
            //+ "&territorio=" + ValidationHelper.GetString(ScriptHelper.GetString("territorio"), "")
            var territorioparam = QueryHelper.GetInteger("territorio",0);
            URLHelper.Redirect("User_Edit_Frameset.aspx?userid=" + actionArgument + "&siteid=" + SiteID + "&territorio=" + territorioparam);
